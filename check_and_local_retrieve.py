...
SLEEP_SEC=1 #interval checking PACS data completeness 
...
def have_new_scan_and_ready_for_retrieve(uwo_username,uwo_password,study_desc,study_date):
    '''
    pre=get PI+Today's NumberOfStudyRelatedInstances
    if pre not empty:
        wait 20-30 seconds
    ...
    matching_key= "-m StudyDescription='{}' -m StudyDate='{}'".format(study_desc,study_date)
...
def main(uwo_cred_id,study_desc,study_date):
    ...
    isready = have_new_scan_and_ready_for_retrieve(uwo_username,uwo_password,study_desc,study_date)

    print "isready: {}".format(isready)
    if isready:
        print "scan on {} is ready to download".format(study_date)
	sys.exit(0)
    else:
        print "scan on {} is not yet ready to download".format(study_date)
	sys.exit(2)
...
#        cmd="{} {} {}".format(local_script,study_date,uwo_cred_id)
#        print 'cmd local_script: {}'.format(cmd)
#        try:
#            stdout_stderr = subprocess.check_output(cmd, stderr=subprocess.STDOUT, shell=True)
#            print stdout_stderr
#        except subprocess.CalledProcessError as e:
#            print 'local script returned non-zero exit status'
            
    
if __name__=="__main__":

    if len(sys.argv) == 3:
	uwo_cred_id=sys.argv[1]
	study_desc=sys.argv[2]
	...
	study_desc=sys.argv[2]
        ...
        print ("Return value of 0 if scans are ready to download")
        print ("Usage: python " + os.path.basename(__file__)+ " < uwo credential id >  < study desc match> [date (optional, default: today)]")
#        print ("Example: python check_and_local_retrieve.py ak /project/6007967/yinglilu/autobids/bin/procNewScans 20171116")
        sys.exit(1)

    
    main(uwo_cred_id,study_desc,study_date)
